...
local SoapLibrary = game:GetObjects("rbxassetid://10804731440")[1]



if gethui then
	SoapLibrary.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(SoapLibrary)
	SoapLibrary.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	SoapLibrary.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	SoapLibrary.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == SoapLibrary.Name and Interface ~= SoapLibrary then
			Interface.Enabled = false
			Interface.Name = "SoapLibrary-Old"
		end
	end
else
...
local Main = SoapLibrary.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

SoapLibrary.DisplayOrder = 100
...
local Notifications = SoapLibrary.Notifications

local SelectedTheme = SoapLibraryLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = SoapLibraryLibrary.Theme[ThemeName]
	for _, obj in ipairs(SoapLibrary:GetDescendants()) do
	...
	SoapLibrary.Main.BackgroundColor3 = SelectedTheme.Background
	SoapLibrary.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	SoapLibrary.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	SoapLibrary.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow
	
	SoapLibrary.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	SoapLibrary.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	SoapLibrary.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	table.foreach(Data, function(FlagName, FlagValue)
		if SoapLibraryLibrary.Flags[FlagName] then
			spawn(function() 
				if SoapLibraryLibrary.Flags[FlagName].Type == "ColorPicker" then
					SoapLibraryLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if SoapLibraryLibrary.Flags[FlagName].CurrentValue or SoapLibraryLibrary.Flags[FlagName].CurrentKeybind or SoapLibraryLibrary.Flags[FlagName].CurrentOption or SoapLibraryLibrary.Flags[FlagName].Color ~= FlagValue then SoapLibraryLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			SoapLibraryLibrary:Notify({Title = "Flag Error", Content = "SoapLibrary was unable to find '"..FlagName.. "'' in the current script"})
		end
	end)
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(SoapLibraryLibrary.Flags) do
...
function SoapLibraryLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= SoapLibraryLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("SoapLibrary | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if SoapLibrary.Name == "SoapLibrary" then
...
function Hide()
	Debounce = true
	SoapLibraryLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightShift", Duration = 7})
...
function SoapLibraryLibrary:CreateWindow(Settings)
	...
	pcall(function()
		...
		if not isfolder(SoapLibraryFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(SoapLibraryFolder.."/Discord Invites") then
			makefolder(SoapLibraryFolder.."/Discord Invites")
		end
		if not isfile(SoapLibraryFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(SoapLibraryFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"SoapLibrary RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(SoapLibraryFolder.."/Key System") then
			makefolder(SoapLibraryFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			...
			if not Success then
				print("SoapLibrary | "..Settings.KeySettings.Key.." Error " ..tostring(Response))
			end
		end
		...
		if isfile(SoapLibraryFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(SoapLibraryFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				if KeyMain.Input.InputBox.Text == Settings.KeySettings.Key then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(SoapLibraryFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
						SoapLibraryLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					SoapLibraryLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("SoapLibrary | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					if Dropdown.Selected.Text ~= Option then
						...
						if not Success then
							...
							print("SoapLibrary | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					SoapLibraryLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("SoapLibrary | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					SoapLibraryLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("SoapLibrary | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					SoapLibraryLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("SoapLibrary | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					SoapLibraryLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function SoapLibraryLibrary:Destroy()
	SoapLibrary:Destroy()
end
...
function SoapLibraryLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				SoapLibraryLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

return SoapLibraryLibrary
